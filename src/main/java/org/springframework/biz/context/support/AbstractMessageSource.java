...
public abstract class AbstractMessageSource extends MessageSourceSupport implements SpringMessageSource {
	
	protected SpringMessageSource parentMessageSource;
	protected Properties commonMessages;
	protected boolean useCodeAsDefaultMessage = false;

	@Override
	public SpringMessageSource getParentMessageSource() {
		return parentMessageSource;
	}
	
	@Override
	public void setParentMessageSource(SpringMessageSource parent) {
	...
	public String getMessage(MessageSourceResolvable resolvable) throws NoSuchMessageException {
		...
		for (String code : codes) {
			String msg = getMessageInternal(code, resolvable.getArguments());
			if (msg != null) {
				return msg;
			}
		}
		...
		if (defaultMessage != null) {
			return renderDefaultMessage(defaultMessage, resolvable.getArguments());
		}
		if (codes.length > 0) {
		...
		throw new NoSuchMessageException(codes.length > 0 ? codes[codes.length - 1] : null);
	}
	...
	protected String getMessageInternal(String code, Object[] args) {
		...
		if (commonMessages != null) {
			...
			if (commonMessage != null) {
				return formatMessage(commonMessage, args);
			}
		}
	...
	protected String getMessageFromParent(String code, Object[] args) {
		SpringMessageSource parent = getParentMessageSource();
		if (parent != null) {
