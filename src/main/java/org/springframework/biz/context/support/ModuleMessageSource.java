...
import org.springframework.biz.io.PropertiesResource;
...
public class ModuleMessageSource extends AbstractMessageSource implements BeanClassLoaderAware{
	 ...
	 */
	protected final ConcurrentMap<String, PropertiesResource> cachedPropertiesResources = new ConcurrentHashMap<String, PropertiesResource>();
	 ...
	 */
	protected final ConcurrentMap<PropertiesResource, Map<String, MessageFormat>> cachedBundleMessageFormats = new ConcurrentHashMap<PropertiesResource, Map<String, MessageFormat>>();
	...
	protected List<Resource> calculateResourcesForBasename(String basename) {
		...
		try {
			...
			resources = SpringResourceUtils.getResources(basename + PROPERTIES_SUFFIX);
			if (resources == null || resources.length == 0 ) {
				resources = SpringResourceUtils.getResources(basename + XML_SUFFIX);
			}
		} catch (IOException e) {
